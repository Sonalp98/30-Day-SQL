...
VALUES 
  ...
  with cte as(select f.name,count(*) as total from countries c
  join families f
  on  f.family_size > c.min_size 
  group by f.name)
  select max(total) as max from cte;
   
  -- select max from family as discount is more for more members so for highest discount use max family size
  select count(*) as max from countries 
