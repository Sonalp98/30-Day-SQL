...
select * from flights;
select f.cid,f.origin,f2.Destination
from flights f
join flights f2 on f.Destination= f2.origin;
drop table sales
CREATE TABLE sales 
(
    order_date date,
...
select * from sales;
-- Count of new customers added each month
-- first visit of each customer
select order_date , count(distinct customer) as no from(select order_date, customer, row_number() over(partition by customer order by order_date) as rn
from sales ) s
where rn=1
group by order_date;
select * from(select order_date, customer, row_number() over(partition by customer order by order_date) as rn
from sales ) s
where rn=1;