...
select * from movies;
select * from reviews;
with cte as (Select genre,title, avg(rating) as avg_rating , dense_rank() over(partition by genre order by avg(rating) desc) as rn from movies m
join reviews r
on m.id=r.movie_id
group by genre,title)
select title,genre, 
case when round(avg_rating) = "1" then "*"
when round(avg_rating) = "2" then "**"
when round(avg_rating) = "3" then "***"
when round(avg_rating) = "4" then "****"
when round(avg_rating) = "5" then "*****" end as avg_rating
from cte 
where rn =1;
-- Use repeat function instead
with cte as (Select genre,title, avg(rating) as avg_rating , dense_rank() over(partition by genre order by avg(rating) desc) as rn from movies m
join reviews r
on m.id=r.movie_id
group by genre,title)
